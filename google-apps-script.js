...
// This script will handle web requests and add data to Google Sheets
...
function handleLead(data) {
  try {
    ...
    const rowData = [
      ...
      data.utm?.utm_term || '',
      data.utm?.utm_content || '',
      data.source || '',
      data.variant || '',
      data.calculatorResults ? 'Yes' : 'No',
      data.calculatorResults?.inputs?.systemSizeKw || '',
      data.calculatorResults?.results?.totalRebates || '',
      data.calculatorResults?.results?.finalPrice || '',
      leadId,
      'New'
    ];
