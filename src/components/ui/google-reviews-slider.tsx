...
type ReviewSource = "google" | "solarquotes";
...
export function GoogleReviewsSlider() {
  ...
  // Build the combined review list: 3 SolarQuotes + 3 Google (5-star)
  const combinedReviews: Review[] = React.useMemo(() => {
    const sq = solarQuotesReviews
      .filter((r) => r.rating === 5)
      .slice(0, 3)
      .map((r) => ({ ...(r as any), source: "solarquotes" as const }));

    const g = googleReviews
      .filter((r) => r.rating === 5)
      .slice(0, 3)
      .map((r) => ({ ...(r as any), source: "google" as const }));
  ...
  const currentReview = combinedReviews[currentIndex];

  const goPrev = () => {
    setCurrentIndex((prev) => (prev - 1 + combinedReviews.length) % combinedReviews.length);
  };
        ...
        <div className="relative">
                          ...
                          <div className="flex items-center gap-3 shrink-0">
                            ...
                            <div>
                              ...
                              <p className="text-sm text-gray-500 dark:text-gray-400">
                                {currentReview.source === "google" ? "Google Review" : "SolarQuotes Review"}
                              </p>
                            </div>
                          </div>
                        </div>
          ...
          >
            ...
            <div className="flex items-center justify-center gap-4">
              ...
              <a
                href="https://www.solarquotes.com.au/solar-reviews/"
                target="_blank"
                rel="noopener noreferrer"
            ...
            <p className="text-xs text-gray-400 dark:text-gray-500 mt-2">
              *Reviews shown from Google My Business and SolarQuotes.
            </p>
          </motion.div>
        </div>
      </div>
